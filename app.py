...
def setup():
    ...
    set_technology_dropdown_1(False)
...
def set_technology_dropdown_1(event):
    if event:
        if event.event_name == 'change':
            tech = list(DATA[event.widget.selected]["addresses"][side_dd_1.selected].keys())
        else:
            tech = list(DATA[env_radio_1.selected]["addresses"][event.widget.selected].keys())
    else:
        tech = list(DATA[env_radio_1.selected]["addresses"][side_dd_1.selected].keys())
    technology_dd_1.items = tech
...
def send_request(next=False):
    ...
    output_str = ''
    if next:
        feasibility_response = next_available(env_radio_1.selected, technology_dd_1.selected, side_dd_1.selected)
        o = output_tb_1
    else:
...
env_radio.add_event_listener('change', set_technology_dropdown)

side_lbl = gp.Label(check_feasibility_on_a_specific_address_tab, 'Select Side')
side_dd = gp.Dropdown(check_feasibility_on_a_specific_address_tab, ['OPT', 'SDL'])
side_dd.selected_index = 0
side_dd.add_event_listener('select', set_technology_dropdown)

technology_lbl = gp.Label(check_feasibility_on_a_specific_address_tab, 'Select Technology')
technology_dd = gp.Dropdown(check_feasibility_on_a_specific_address_tab, [])
technology_dd.add_event_listener('select', set_address_dropdown)
...
check_feasibility_on_a_specific_address_tab.set_grid(7, 2)
check_feasibility_on_a_specific_address_tab.add(env_lbl, 1, 1)
check_feasibility_on_a_specific_address_tab.add(env_radio, 1, 2)

check_feasibility_on_a_specific_address_tab.add(side_lbl, 2, 1)
check_feasibility_on_a_specific_address_tab.add(side_dd, 2, 2)

check_feasibility_on_a_specific_address_tab.add(technology_lbl, 3, 1)
check_feasibility_on_a_specific_address_tab.add(technology_dd, 3, 2)

check_feasibility_on_a_specific_address_tab.add(select_address_lbl, 4, 1)
check_feasibility_on_a_specific_address_tab.add(select_address_dd, 4, 2)

check_feasibility_on_a_specific_address_tab.add(container, 5, 2)
check_feasibility_on_a_specific_address_tab.add(padding, 6, 1)
check_feasibility_on_a_specific_address_tab.add(output_tb, 7, 1, column_span=2, fill=True)
...
env_radio_1.add_event_listener('change', set_technology_dropdown_1)

side_lbl_1 = gp.Label(next_available_address_tab, 'Select Side')
side_dd_1 = gp.Dropdown(next_available_address_tab, ['OPT', 'SDL'])
side_dd_1.selected_index = 0
side_dd_1.add_event_listener('select', set_technology_dropdown_1)

technology_lbl_1 = gp.Label(next_available_address_tab, 'Select Technology')
technology_dd_1 = gp.Dropdown(next_available_address_tab, [])
...
next_available_address_tab.add(side_lbl_1, 2, 1)
next_available_address_tab.add(side_dd_1, 2, 2)
next_available_address_tab.add(technology_lbl_1, 3, 1)
next_available_address_tab.add(technology_dd_1, 3, 2)
